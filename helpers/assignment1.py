...
def generate_instance(n_nodes, seed, n_vehicles=1):
    ...
    CENTER_MEAN_QBOX = 0.25, 0.75
    CENTER_STD_QBOX = 0.1, 0.15
...
def solve_model(solver, model):
    ...
    if (
        ...
        raise RuntimeError(
            "Solver failed in %.2fs with status '%s' and termination condition '%s'"
...
def plot_solution(coords, model):
    ...
    adjacency = {
        i: {j: round(model.arc[i, j]()) for j in model.nodes} for i in model.nodes
    }
    ...
    routes = [
        nx.shortest_path(G, i, j) for j in t for i in s if nx.has_path(G, i, j)
    ...
    node_colors = (
        ["green"] * len(s)
        + ["blue"] * (coords.size - len(s) - len(t))
        + ["red"] * len(t)
    )

    # Plot instance and routes on a coordinate grid
    edge_labels = {
        (i, j): round(model.node_load[i]())
        for i in model.nodes
        for j in model.nodes
        if round(model.arc[i, j]()) == 1
    }
    ...
    # Extract the string describing the expression used for the objective
    objective_str = str(model.obj.expr).replace("model.", "")
    ...
    for r, route in enumerate(routes, 1):
        ...
        ax[r].scatter(
            ...
            node_loads,
            s=300,
            c=[node_colors[x - 1] for x in route],
            alpha=1,
        )
        for i in route:
            ax[r].annotate(
                i,
                (arrival_times[i], node_loads[i]),
                fontsize=11,
                color="white",
                ha="center",
                va="center",
        ...
        ax[r].set(
            xlabel="time (minutes)",
            ylabel="load ($10^3$ kg)",
            title="Total vehicle load over time",
        )
